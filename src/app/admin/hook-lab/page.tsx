...
import AdminHookLab from "@/components/AdminHookLab";
import AdminLogoutButton from "@/components/AdminLogoutButton";
import { requireAdminSession } from "@/lib/admin-auth";
import { getConfiguredAiModels } from "@/lib/ai-models";
import { videos } from "@/lib/content";

export const metadata: Metadata = {
  title: "Hook Lab",
  description: "Private Vanta Orbit Media hook performance lab.",
};
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black px-4 pb-20 pt-28 sm:px-6 lg:px-8">
      ...
      <section className="relative mx-auto max-w-7xl">
        <div className="flex flex-col gap-6 rounded-[2rem] border border-white/10 bg-black/55 p-6 shadow-[0_0_55px_rgba(124,58,237,0.18)] backdrop-blur-xl sm:flex-row sm:items-end sm:justify-between sm:p-8">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-zinc-300 sm:text-base">
              Track published hook performance, find repeatable audience patterns, and feed winning hook signals back into future video ideas.
            </p>
          </div>
          <AdminLogoutButton />
        </div>
        ...
        <div className="mt-8">
          <AdminHookLab videos={videos} aiModels={getConfiguredAiModels()} />
        </div>
      </section>
    </main>
  );
