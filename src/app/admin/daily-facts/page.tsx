...
import { listDailyFacts } from "@/lib/daily-space-facts";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Daily Space Facts",
  description: "Private Vanta Orbit Media daily space facts admin.",
};

export default async function AdminDailyFactsPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black px-4 pb-20 pt-28 sm:px-6 lg:px-8">
      ...
      <section className="relative mx-auto max-w-7xl">
        <div className="flex flex-col gap-6 rounded-[2rem] border border-white/10 bg-black/55 p-6 shadow-[0_0_55px_rgba(124,58,237,0.18)] backdrop-blur-xl sm:flex-row sm:items-end sm:justify-between sm:p-8">
          <div>
            <div className="flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-violet-200">
              ...
              <span>/</span>
              <span>Daily Space Facts</span>
            </div>
            <h1 className="mt-4 font-display text-3xl font-bold uppercase tracking-[0.14em] text-white sm:text-4xl">
              Daily Space Facts
            </h1>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-zinc-300 sm:text-base">
              Schedule short homepage facts, review AI draft packs, publish full posts, and export social cards.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link
              href="/admin/daily-facts/import"
              className="inline-flex min-h-11 items-center justify-center rounded-full border border-violet-200/35 px-5 text-xs font-bold uppercase tracking-[0.16em] text-white transition hover:border-violet-100 hover:bg-violet-400/10"
            >
              Import JSON
            </Link>
            <AdminLogoutButton />
        ...
        <div className="mt-8">
          <AdminDailyFacts initialFacts={facts} />
        </div>
      </section>
    </main>
  );
